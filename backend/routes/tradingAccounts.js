...
const isValidObjectId = (id) => {
  return (
    id && id !== "undefined" && id !== "null" && /^[a-fA-F0-9]{24}$/.test(id)
  );
};
...
router.get("/user/:userId", async (req, res) => {
  try {
    ...
    const accounts = await TradingAccount.find({ userId })
      .populate(
        "accountTypeId",
        "name description minDeposit leverage exposureLimit isDemo",
      )
      .sort({ createdAt: -1 });
    res.json({ success: true, accounts });
  } catch (error) {
...
router.post("/", async (req, res) => {
  try {
    ...
    if (!isValidObjectId(userId) || !isValidObjectId(accountTypeId)) {
      return res
        .status(400)
        .json({ message: "Invalid user ID or account type ID" });
    }
    ...
    if (!accountType || !accountType.isActive) {
      return res
        .status(400)
        .json({ message: "Invalid or inactive account type" });
    }
    ...
    // Determine initial balance - Demo accounts get auto-funded with non-refundable balance
    const initialBalance = accountType.isDemo
      ? accountType.demoBalance || 10000
      : 0;
 ...
 */
    ...
    // 🔔 CREATE NOTIFICATION (FOR BOTH LIVE & DEMO)
    try {
      const notification = await Notification.create({
        userId: userId,
        title: "Account Created",
        message: `New ${accountType.name} account created successfully`,
        type: "ACCOUNT",
        relatedId: tradingAccount._id,
      });

      console.log("✅ Account Notification Created");
...
router.put("/:id/change-pin", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error changing PIN", error: error.message });
  }
});
...
router.put("/:id/reset-pin", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error resetting PIN", error: error.message });
  }
});
...
router.post("/account-transfer", async (req, res) => {
  try {
    ...
    if (!fromAccountId || !toAccountId) {
      return res
        .status(400)
        .json({ message: "Both source and target accounts are required" });
    }
    ...
    if (fromAccount.userId.toString() !== userId) {
      return res
        .status(403)
        .json({ message: "Unauthorized access to source account" });
    }
    ...
    if (fromAccount.isDemo) {
      return res
        .status(400)
        .json({ message: "Cannot transfer funds from a demo account" });
    }
    ...
    if (toAccount.userId.toString() !== userId) {
      return res
        .status(403)
        .json({ message: "Unauthorized access to target account" });
    }
    ...
    if (toAccount.isDemo) {
      return res
        .status(400)
        .json({ message: "Cannot transfer funds to a demo account" });
    }
    ...
    console.log(
      `[Account Transfer] ${fromAccount.accountId} -> ${toAccount.accountId}: $${amount}`,
    );
...
router.put("/:id/archive", async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "Error archiving account",
        error: error.message,
      });
  }
});
...
router.put("/:id/unarchive", async (req, res) => {
  try {
    ...
    if (account.status !== "Archived") {
      return res
        .status(400)
        .json({ success: false, message: "Account is not archived" });
    }
  ...
  try {
    ...
    if (account.status !== "Archived") {
      return res.status(400).json({
        success: false,
        message:
          "Only archived accounts can be permanently deleted. Archive the account first.",
      });
    }
...
router.post("/:id/reset-demo", async (req, res) => {
  try {
    ...
    if (!account.isDemo) {
      return res
        .status(400)
        .json({ success: false, message: "Only demo accounts can be reset" });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: "Error resetting demo account",
        error: error.message,
      });
  }
});
