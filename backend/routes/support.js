  ...
  try {
    ...
    try {
      ...
      if (io) {
        ...
        console.log("📡 Support Notification Sent Real-time");
      } else {
...
router.post("/reply/:ticketId", async (req, res) => {
  try {
    ...
    if (senderType !== "ADMIN") {
      if (!senderId) {
        return res.status(400).json({
          success: false,
          message: "Sender ID is required for user replies",
        });
      }
      const user = await User.findById(senderId);
      if (!user) {
    ...
    // Re-fetch with populated fields
    const updatedTicket = await SupportTicket.findOne({ ticketId }).populate(
      "userId",
