...
const scaleIn = "animate-[scaleIn_0.3s_ease-out_forwards]";
 ...
 const ProfilePage = () => {
  ...
  const fetchUserBankAccounts = async () => {
    try {
      const res = await fetch(
        `${API_URL}/payment-methods/user-banks/${currentUser._id}`,
      );
      const data = await res.json();
  ...
  const handleDeleteBankAccount = async (id) => {
    ...
    try {
      const res = await fetch(`${API_URL}/payment-methods/user-banks/${id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success) {
  ...
  const fetchLoginHistory = async () => {
    try {
      const res = await fetch(
        `${API_URL}/auth/login-history/${currentUser._id}`,
      );
      const data = await res.json();
      if (data.success) {
      ...
      <div className="min-h-screen flex bg-[#F8F8F8] text-gray-800">    
        ...
        > */}
          ...
          <div className={`${isMobile ? "p-4" : ""} space-y-6`}>
            ...
            {success && (
              <div className="p-3 bg-green-500/20 border border-green-500/50 rounded-lg text-green-500 flex items-center gap-2 text-sm">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-md hover:shadow-green-100 transition">
              <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                ...
                <div className="relative">
                  <div className="h-24 w-24 rounded-full bg-[#008D00] text-white flex items-center justify-center text-2xl font-semibold">
                    {profile.fullName
                      ?.split(" ")
                      .map((n) => n.charAt(0))
                      .slice(0, 2)
                  ...
                  {editing && (
                    <button className="absolute -bottom-1 -right-1 h-8 w-8 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200">
                      <Camera size={14} />
                    </button>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-2 text-sm">
                  {/* Account Level (static for now) */}
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500">Account Level:</span>
                    <span className="text-xs px-2 py-0.5 rounded-md bg-yellow-100 text-yellow-600">
                      Gold
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {currentUser.kycApproved ? "Verified" : "In Progress"}
                    </span>
                  </div>
                </div>
              </div>
            ...
            {/* Tabs */}
            <div className="inline-flex h-9 items-center rounded-lg p-[3px] w-full justify-start overflow-x-auto bg-[#f1f5f9] text-[#64748b]">
              {[
                ...
                <button
                  ...
                  className={`
        flex-1 inline-flex items-center justify-center text-bold
        rounded-md border border-transparent 
        px-2 py-1 text-sm font-medium 
        whitespace-nowrap transition-all gap-1.5
      ...
      `}
            ...
            {activeTab === "profile" && (
              <div className="flex flex-col gap-5">
                <div
                  className={`${isDarkMode ? "bg-dark-800 border-gray-800" : "bg-white border-gray-200 shadow-sm"} rounded-2xl ${isMobile ? "p-4" : "p-6"} border transition-all duration-300 hover:shadow-lg ${isDarkMode ? "hover:shadow-black/20" : "hover:shadow-gray-200"}`}
                >
                  <div className="flex flex-row justify-between">
                    <h3
                      className={`font-semibold ${isMobile ? "mb-4 text-base" : "mb-6"} flex items-center gap-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}
                    >
                      <User size={18} className="text-accent-green" /> Personal
                      Information
                    </h3>
                    ...
                    {!editing ? (
                      <button
                        ...
                        className="flex items-center gap-2 bg-green-600 hover:bg-green-70 text-white px-4 py-2 rounded-lg font-medium hover:bg-accent-green/90"
                      >
                        <Edit2 size={16} />
                        Edit Profile
                      </button>
                  ...
                  >
                    <div className="space-y-1.5">
                      <label
                        className={`text-sm font-medium block ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                      >
                        Full Name
                      </label>
                      {editing ? (
                    ...
                    <div className="space-y-1.5">
                      <label
                        className={`text-sm font-medium flex items-center gap-2 ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                      >
                ...
                >
                  <h3
                    className={`font-semibold ${isMobile ? "mb-4 text-base" : "mb-6"} flex items-center gap-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}
                  >
                    <Building2 size={18} className="text-blue-400" /> Bank
                  ...
                  >
                    ...
                    <div
                      className={`${isMobile ? "" : "col-span-2"} space-y-1.5`}
                    >
                      <label
                        className={`text-sm font-medium block ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                      >
                ...
                >
                  <h3
                    className={`font-semibold ${isMobile ? "mb-4 text-base" : "mb-6"} flex items-center gap-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}
                  >
                    <Smartphone size={18} className="text-purple-400" /> UPI
                    Details
                  </h3>
                  ...
                  {!editing &&
                    ...
                    !profile.upiId && (
                      <div
                        className={`mt-4 p-4 rounded-xl border ${isDarkMode ? "bg-yellow-500/10 border-yellow-500/30" : "bg-yellow-50 border-yellow-200"}`}
                      >
                        <p
                ...
                >
                  ...
                  <p
                    className={`text-sm mb-4 ${isDarkMode ? "text-gray-500" : "text-gray-600"}`}
                  >
                    Add bank accounts or UPI IDs for withdrawals. Accounts
                    require admin approval before use.
                  </p>

                  {userBankAccounts.length === 0 ? (
                    <div
                      className={`p-6 rounded-xl text-center ${isDarkMode ? "bg-dark-700" : "bg-gray-50"}`}
                    >
                      <CreditCard
                        size={32}
                    ...
                    <div className="space-y-3">
                      {userBankAccounts.map((acc) => (
                        <div
                          key={acc._id}
                          className={`p-4 rounded-xl border transition-all duration-200 hover:scale-[1.01] ${isDarkMode ? "bg-dark-700 border-gray-700" : "bg-gray-50 border-gray-200"}`}
                        >
                          <div
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-10 h-10 rounded-xl flex items-center justify-center ${acc.type === "Bank Transfer" ? "bg-blue-500/20" : "bg-purple-500/20"}`}
                              >
                              ...
                              <div>
                                ...
                                {acc.type === "Bank Transfer" ? (
                                  <p
                                    className={`text-sm mt-0.5 ${isDarkMode ? "text-gray-500" : "text-gray-600"}`}
                                  >
                                    A/C: {acc.accountNumber} | IFSC:{" "}
                                    {acc.ifscCode}
                                  </p>
                                ) : (
                                  <p className="text-purple-400 text-sm font-mono mt-0.5">
                                    {acc.upiId}
                                  </p>
                                )}
                                {acc.rejectionReason && (
                                  <p className="text-red-400 text-xs mt-1">
                                    Reason: {acc.rejectionReason}
                                  </p>
                                )}
                              </div>
                            </div>
          ...
          {activeTab === "kyc" && (
            <div
              className={`${isDarkMode ? "bg-dark-800 border-gray-800" : "bg-white border-gray-200 shadow-sm"} rounded-2xl ${isMobile ? "p-4" : "p-6"} border transition-all duration-300 hover:shadow-lg ${isDarkMode ? "hover:shadow-black/20" : "hover:shadow-gray-200"} mt-5`}
            >
              <h3
                className={`font-semibold ${isMobile ? "mb-4 text-base" : "mb-4"} flex items-center gap-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}
              >
                <FileCheck size={18} className="text-cyan-400" /> KYC
                Verification
              </h3>
              ...
              {kycStatus ? (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="border-2 border-dashed border-gray-600 rounded-lg p-4 text-center hover:border-accent-green transition-colors">
                      {kycForm.selfieImage ? (
                        <div className="relative">
                          <img
                            src={kycForm.selfieImage}
                            alt="Selfie"
                            className="max-h-32 mx-auto rounded"
                          />
                          <button
                            onClick={() =>
                              setKycForm({ ...kycForm, selfieImage: "" })
                            }
                            className="absolute top-0 right-0 p-1 bg-red-500 rounded-full"
                          >
                            <X size={14} className="text-white" />
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      className="flex-1 py-3  bg-gray-200 text-black rounded-lg hover:bg-gray-400"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      disabled={kycLoading}
                      className="flex-1 py-3 bg-green-600 hover:bg-green-700   text-white font-medium rounded-lg hover:bg-accent-green/90 disabled:opacity-50"
                    >
                ...
                <div className="text-center py-6">
                  ...
                  <button
                    ...
                    className="px-6 py-3 bg-green-600 hover:bg-green-700  text-white font-medium rounded-lg hover:bg-accent-green/90"
                  >
          ...
          {activeTab === "security" && (
            ...
            >
              ...
              <div className="space-y-2">
                <div
                  className={`flex ${isMobile ? "flex-col gap-2" : "items-center justify-between"} p-4 rounded-xl transition-all duration-200 ${isDarkMode ? "bg-dark-700/50 hover:bg-dark-700" : "bg-gray-50 hover:bg-gray-100"}`}
                >
                  <div>
                    <p
                      className={`font-medium ${isDarkMode ? "text-white" : "text-gray-900"}`}
                  ...
                  <button
                    ...
                    className={`${isMobile ? "w-full" : ""} px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-xl text-sm font-medium hover:bg-accent-green/20 transition-all duration-200 active:scale-98`}
                  >
                    Change Password
                  </button>
                </div>
                ...
                >
                  ...
                  <button
                    ...
                    className={`${isMobile ? "w-full" : ""} px-4 py-2 bg-blue-500/10 text-blue-500 rounded-xl text-sm font-medium hover:bg-blue-500/20 transition-all duration-200 active:scale-98`}
                  >
                    Enable
                  </button>
                </div>
                ...
                >
                  ...
                  <button
                    ...
                    className={`${isMobile ? "w-full" : ""} px-4 py-2 bg-purple-500/10 text-purple-500 rounded-xl text-sm font-medium hover:bg-purple-500/20 transition-all duration-200 active:scale-98`}
                  >
                    View
                  </button>
                </div>
          ...
          {showBankForm && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-[fadeIn_0.2s_ease-out]">
              <div
                className={`${isDarkMode ? "bg-dark-800 border-gray-700" : "bg-white border-gray-200"} rounded-2xl w-full max-w-md border shadow-2xl animate-[scaleIn_0.2s_ease-out]`}
              >
                <div
                ...
                >
                  ...
                  <button
                    ...
                    className={`p-2 rounded-lg transition-colors ${isDarkMode ? "text-gray-400 hover:text-white hover:bg-dark-700" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100"}`}
                  >
                    <X size={20} />
                  </button>
                </div>
                <div className="p-4 space-y-4">
                    ...
                    <>
                      <div>
                        ...
                        <input
                          ...
                          placeholder="e.g., HDFC Bank"
                          className="w-full bg-gray-100 border border-gray-600 rounded-lg px-3 py-2 text-black"
                        />
                      </div>
                      <div>
                        <label className="text-gray-400 text-sm block mb-1">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder="e.g., HDFC0001234"
                          className="w-full bg-gray-100 border border-gray-600 rounded-lg px-3 py-2 text-black uppercase"
                        />
                      </div>
                      <div>
                        <label className="text-gray-400 text-sm block mb-1">
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => setShowBankForm(false)}
                      className="flex-1 py-2 bg-gray-200 text-black rounded-lg hover:bg-gray-400"
                    >
                      Cancel
                    </button>
                    <button
          ...
          {showPasswordModal && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-[fadeIn_0.2s_ease-out]">
              <div
                className={`${isDarkMode ? "bg-dark-800 border-gray-700" : "bg-white border-gray-200"} rounded-2xl w-full max-w-md border shadow-2xl overflow-hidden animate-[scaleIn_0.2s_ease-out]`}
              >
                <div
                ...
                <div className="p-4 space-y-4">
                  {passwordMessage.text && (
                    <div
                      className={`p-3 rounded-xl text-sm ${passwordMessage.type === "error" ? "bg-red-500/20 text-red-400" : "bg-green-500/20 text-green-400"}`}
                    >
                      {passwordMessage.text}
                    </div>
                  )}
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      className={`flex-1 py-3 rounded-xl font-medium transition-all duration-200 active:scale-98 ${isDarkMode ? "bg-green-600 text-gray-400 hover:bg-green-700" : "bg-gray-200 text-gray-600 hover:bg-gray-300"}`}
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      className="flex-1 py-3 bg-green-600 text-white hover:bg-green-700 font-medium rounded-xl hover:bg-accent-green/90 disabled:opacity-50 transition-all duration-200 active:scale-98"
                    >
            ...
            >
              ...
              <div className="p-4 max-h-96 overflow-y-auto">
                {loginHistory.length === 0 ? (
                  <div
                    className={`text-center py-8 ${isDarkMode ? "text-gray-500" : "text-gray-600"}`}
                  >
                    <Clock size={48} className="mx-auto mb-3 opacity-50" />
                    <p>Login history tracking coming soon</p>
                    <p className="text-sm mt-1">
                      Your login activity will be displayed here
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {loginHistory.map((item, index) => (
                      <div
                        key={index}
                        className={`p-3 rounded-xl ${isDarkMode ? "bg-dark-700" : "bg-gray-50"}`}
                      >
