 ...
 */
  ...
  useEffect(() => {
    ...
    newSocket.on("connect", () => {
      ...
      newSocket.on("newAnnouncement", (data) => {
 ...
 */
    ...
    return () => {
      newSocket.off("newAnnouncement"); // ✅ remove listener
      ...
      <div className="flex items-center gap-4 relative">
        ...
        <div className="relative" ref={bellRef}>
          <div
            ...
            className="p-2 rounded-full bg-gray-200 hover:bg-gray-200 transition cursor-pointer"
          >
              ...
              <div className="max-h-72 overflow-y-auto">
                {announcements.length === 0 ? (
                  ...
                  announcements.map((item) => (
                    <div
                      key={item._id}
                      className={`p-3 border-b transition ${getPriorityStyle(item.priority)}`}
                    >
                      <p className="font-semibold text-sm text-gray-800 flex items-center">
                        {item.title}

                        {/* 🆕 NEW BADGE (already present) */}
                        {newIds.includes(item._id) && (
                          <span className="ml-2 text-[10px] bg-blue-500 text-white px-2 py-0.5 rounded">
                            NEW
                          </span>
                        ...
                        {/* 🔥 PRIORITY BADGE */}
                        <span
                          className={`ml-2 text-[10px] text-white px-2 py-0.5 rounded ${
                            item.priority === "high"
                              ? "bg-red-500"
